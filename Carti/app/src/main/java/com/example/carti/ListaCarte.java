    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intentModifica = new Intent(getApplicationContext(), AdaugaCarte.class);
                intentModifica.putExtra("carte", carte.get(position));
                idModificat = 1;
                startActivityForResult(intentModifica, 234);
                Toast.makeText(getApplicationContext(), carte.get(position).toString(),Toast.LENGTH_SHORT);
